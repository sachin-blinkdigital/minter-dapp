  ...
  if (publicMintActive) {
    ...
    try {
      ...
      const whitelisted = await contract.methods
        ...
        .call();
      if (!whitelisted) {
        $(".minter_close").removeClass("hidden");
        $('.not_whitelisted').removeClass('hidden');
      ...
      $(".minter_close").removeClass("hidden");
     // $('.not_whitelisted').removeClass('hidden');
      $('.already_minted').removeClass('hidden');
      // console.log(e);
...
async function mint() {
  $(".modal__overlay").css("pointer-events", "none");
  const mintButton = document.getElementById("mintButton");
  mintButton.disabled = true;
  const spinner =
    ...
    try {
      ...
      if (mintTransaction) {
        if (chain === "rinkeby") {
          ...
          $(".minted-container").removeClass("hidden");
          $("#mintContainer").addClass("hidden");
          $(".minter_close").removeClass("hidden");
          $(".modal__overlay").css("pointer-events", "");
        }
        console.log(
          "Minted successfully!",
      ...
      mintButton.innerText = button_public_mint;
      mintButton.disabled = false;
      $("#mintButton").html("Mint Your NFT");
      $(".fail-status").removeClass("hidden");
      setTimeout(() => {
        $(".fail-status").addClass("hidden");
      }, 5000);
      $(".modal__overlay").css("pointer-events", "");
      //console.log(e);
    }
